...
把本地数据（CSV 或 DuckDB 数据库）变成可缩放、可筛选、可分组的交互图表 —— 大文件不卡，原始数据不进上下文。
...
一句话：**让 Agent 帮你把数据变成一张能缩放、能筛选、能分组的交互图表。**

- **Agent 一句话画图** —— `render_chart` 工具读取本地 CSV 或 DuckDB 数据库表，只把列元数据返回给模型，**原始数据永不进入模型上下文**（省 token、不爆上下文）。
...
| 能力 | 说明 |
|---|---|
| **`render_chart` 工具** | 读取本地 CSV / DuckDB 数据库表，返回 `chartId` + 列元数据；原始行不进入模型上下文 |
| **DuckDB 数据源** | 直接打开 `.duckdb` / `.ddb` 数据库文件（只读），列出并读取其中的表；表名可省略时自动取首个表 |
...
| **筛选 + 分组 + SQL** | 结构化条件面板（多值 IN/NOT IN、日期范围）、分组多线（sum/avg/min/max/count）、DuckDB 查询（SELECT/WITH，聚合/排序等二次处理暂拒）；SQL 对 CSV 与 DuckDB 数据源均可用 |
...
dsh-chartlab/
  ...
  src/
    ...
    duckdb.ts           # DuckDB 数据源：只读打开 .duckdb/.ddb，列出/读取数据库表 → 同一列式载荷
    sql.ts              # DuckDB SQL 引擎（懒加载，仅 SELECT/WITH；CSV 与 DuckDB 源共用）
...
`duckdb` 采用懒加载（动态 import）：没有它插件也能正常启动，只有 SQL 查询与 DuckDB 数据源需要该原生模块。
...
**v0.0.1**
- 首版：`render_chart` 工具（CSV 与 DuckDB `.duckdb`/`.ddb` 数据源）+ 二进制数据通道 + 自包含图表页 + Web UI `chartLab` 标签页
