...
- 🗂️ **按会话隔离** —— 每个对话只看自己的图表；删除会话时图表与缓存一并清理。
...
| 🖱️ **交互图表页** | 列选择、折线/散点、框选缩放、x 轴滚轮缩放/拖拽平移、悬浮读数、截图下载（白底可读配色） |
...
| 🗂️ **会话隔离与清理** | `/dsh-chartlab/list?session=<id>` 按会话取图；删除会话自动清理 localStorage 缓存与宿主内存图表 |
...
```powershell
node test/selftest.mjs            # 核心解析 + 二进制路由
node test/apply-test.mjs          # 插件 apply + 工具 + 全路由（含 view 脚本语法校验）
node test/sql-normalize-test.mjs  # 多行 SQL 归一化（21 个用例）
node test/parsewhere-test.mjs     # WHERE 子句往返解析
node test/view-render-check.mjs   # 渲染产物特征断言
```
...
**v0.2.0**
- 插件更名为 **dsh-chartlab**（`dsh-chart` 弃用），路由/存储键/客户端 id 全套同步
- 多行 SQL 自动归一化为单行（注释剥离、字符串感知），附 21 个测试用例
- SQL 输入框支持多行粘贴、样式对齐下拉框；分组下拉框新增清空 ✕ 与 “(可选)” 占位
- 下载截图使用白底可读配色（分割线/图例/轴标签不再看不清）；修复快速移出图表时 tooltip 残留
- 条件筛选多选“清空”只清当前行选择（保留条件行）；空值条件不再触发预览崩溃
